...
在进入正题之前，值得一提的是：**对于这种死锁 Cloud Run 的跨公网访问鉴权问题，我们此前曾经探索过一条更为“硬核”的路径**。当时我们搭建了一台 GCE (Compute Engine) 虚拟机，在上面部署了 Envoy 代理，并编译植入了 `gcp_authn` C++ 滤网插件，通过自建代理节点手动签发 Token 来硬核破局。（有兴趣的同学可以回顾我之前的文章：[《硬核破局！Envoy 编译 gcp_authn 插件：以网关身份动态换取 GCP Identity Token 代理访问受限 Cloud Run》](https://blog.csdn.net/nvd11/article/details/153066928)）。

然而，自建 Envoy 虽然高度可控，但运维成本极高。今天，我们将彻底抛弃厚重的虚拟机与复杂的插件编译，回归到最纯粹的 Serverless 哲学。**本次，我们将为您介绍一个更加云原生、更加优雅且完全免运维的最终解法 —— 巧用 Google 官方全托管的 API Gateway 完美实现鉴权转换。**
